...
set /P tableSpaceExp=<%addr:~6%tablespaces.txt
echo ================================================   
echo start backup  %tableSpaceExp%
echo ================================================
if "%tableSpaceExp%"=="" (
goto end
) else (
...
:demo2
exp "'%USER%/%PASSWORD%@%DATABASE% as sysdba'" tablespaces=(%tableSpaceExp%) file=%DATADIR%\data_%filename%.dmp log=%LOGDIR%/log_%filename%.log
goto end 
:end
rem echo end  .........
